...
CSV_PATH = "dbfs:/FileStore/databricks-medallion-pipeline/data/products.csv"
...
def resolve_config(spark: SparkSession) -> tuple[str, str, str]:
    ...
    csv_path = spark.conf.get("pipeline.csv.products", CSV_PATH)
    bronze_table = spark.conf.get("pipeline.bronze.products", BRONZE_TABLE)
